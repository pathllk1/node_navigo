 ...
 * Get all sales bills
 */
export async function getAllSalesBills(req, res) {
  try {
 ...
 */
export async function getSalesBillById(req, res) {
  try {
    const { firm_id, role } = req.user;
    const { id } = req.params;
 ...
 * Create sales bill
 */
export async function createSalesBill(req, res) {
  try {
    ...
    const result = db.transaction(() => {
      ...
      calculatedItems.forEach(item => {
        ...
        `).run(
          ...
          item.rate, item.total, newBalance, `Sales Bill: ${billNo}`
        );
      });
 ...
 * Update sales bill
 */
export async function updateSalesBill(req, res) {
  try {
    const { firm_id, id: user_id, role } = req.user;
    const { id } = req.params;
    ...
    db.transaction(() => {
      ...
      calculatedItems.forEach(item => {
        ...
        `).run(
          ...
          item.rate, item.total, newBalance, `Sales Bill Updated: ${existingBill.bill_no}`
        );
      });
 ...
 * Delete sales bill
 */
export async function deleteSalesBill(req, res) {
  try {
    const { firm_id, role } = req.user;
    const { id } = req.params;
...
export async function createCreditNote(req, res) {
  try {
    ...
    const originalBill = db.prepare(`
      SELECT * FROM bills WHERE id = ? AND firm_id = ? AND bill_type = 'SALES'
    `).get(id, firm_id);
    ...
    // Create credit note (similar to createSalesBill but with CREDIT_NOTE type)
    // Implementation similar to createSalesBill...
    
    res.status(201).json({ message: 'Credit note created successfully' });
...
export async function createDeliveryNote(req, res) {
  try {
    // Similar to createSalesBill but with DELIVERY_NOTE type
    // No ledger posting, only stock movement
...
export async function convertDeliveryNoteToSales(req, res) {
  try {
    // Convert DELIVERY_NOTE to SALES bill
    // Post to ledger
    
    res.json({ message: 'Delivery note converted to sales bill' });
...
export async function generateBillPDF(req, res) {
  try {
    ...
    // Get bill with items
    const bill = await getSalesBillById(req, { json: () => {} });
    
    // Generate PDF using pdfGenerator utility
    // const pdf = await generateInvoicePDF(bill, firm, items);
...
export async function getSalesByParty(req, res) {
  try {
    ...
    const salesByParty = db.prepare(query).all(...params);

    res.json(salesByParty);
...
export async function getSalesByItem(req, res) {
  try {
    ...
    // This requires parsing items_json from all bills
    // Simplified implementation
    
    res.json({ message: 'Sales by item report not yet implemented' });
