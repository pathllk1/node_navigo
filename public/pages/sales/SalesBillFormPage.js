...
export function SalesBillFormPage() {
  ...
  async function handleSave() {
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(error.error || 'Failed to create sales bill');
      }
    ...
    container.innerHTML = `
      <div class="max-w-6xl mx-auto">
        <div class="flex items-center justify-between mb-6">
          <h1 class="text-3xl font-bold text-gray-900">New Sales Bill</h1>
            ...
            <div class="grid grid-cols-1 md:grid-cols-5 gap-4 mb-4">
              <div class="md:col-span-2">
                ...
                <select name="item_id" class="w-full px-3 py-2 border border-gray-300 rounded-md">
                  ...
                  ${items.map(i => `<option value="${i.id}">${i.item_name} (Stock: ${i.current_stock || 0})</option>`).join('')}
                </select>
              </div>
              <div>
  ...
  function setupEventListeners() {
    container.addEventListener('click', (e) => {
      ...
      else if (cancelBtn) window.router.navigate('/sales');
