/**
 * Purchase Dashboard
 * Manages purchase bills and debit notes
 */
...
export function PurchaseDashboard() {
  ...
  container.className = 'purchase-dashboard p-6';

  async function loadBills() {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(
        `/api/purchase?status=${currentStatus}&page=${currentPage}&sortBy=${sortColumn}&sortOrder=${sortDirection}`,
        { headers: { 'Authorization': `Bearer ${token}` } }
      );

      if (!response.ok) throw new Error('Failed to load purchase bills');
  ...
  async function loadSummary() {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch('/api/purchase/reports/summary', {
        headers: { 'Authorization': `Bearer ${token}` }
      });
      ...
      <div class="max-w-7xl mx-auto">
        <div class="flex items-center justify-between mb-6">
          <div>
            ...
            <p class="text-gray-600 mt-1">Manage purchase bills and supplier invoices</p>
          </div>
          <button 
  ...
  function renderStatsCards() {
    return `
      <div class="bg-blue-50 border border-blue-200 rounded-lg p-4">
        <div class="text-sm text-blue-600 font-medium">Total Purchase</div>
        <div class="text-2xl font-bold text-blue-900 mt-1">₹${(summary.total_purchase || 0).toFixed(2)}</div>
      </div>
  ...
  function renderBillsTable() {
    ...
    return DataTable({
      ...
      emptyMessage: 'No purchase bills found. Click "New Purchase Bill" to create one.'
    });
  }
  ...
  function setupEventListeners() {
    ...
    container.addEventListener('click', async (e) => {
      ...
      if (addBtn) {
        window.router.navigate('/purchase/new');
      } else if (viewBtn) {
        window.router.navigate(`/purchase/${viewBtn.dataset.id}`);
      } else if (editBtn) {
        window.router.navigate(`/purchase/${editBtn.dataset.id}/edit`);
      } else if (printBtn) {
        window.open(`/api/purchase/${printBtn.dataset.id}/pdf`, '_blank');
      } else if (deleteBtn) {
  ...
  async function deleteBill(billId) {
    showConfirm({
      ...
      onConfirm: async () => {
        try {
          const token = localStorage.getItem('token');
          const response = await fetch(`/api/purchase/${billId}`, {
            method: 'DELETE',
            headers: { 'Authorization': `Bearer ${token}` }
          });
